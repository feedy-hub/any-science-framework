...
- UI 只讀展示 `workspace/` 中的卡片、結果、知識和 hook 日誌。
- 唯一寫入口是 `/api/inbox`，只會寫入 `workspace/inbox/`。
- UI 輸入被視為半信任內容，仍需總管按正常流程處理。
- server 只綁定 `127.0.0.1`，並保留 Host / Origin 校驗。
- 安裝時如果目標文件已存在，會先建立 `.bak.<timestamp>` 備份。
...
轉寫結果會先讓你確認，確認後只寫入 `workspace/inbox/`，不會直接修改任何 idea、實驗或結果文件。
